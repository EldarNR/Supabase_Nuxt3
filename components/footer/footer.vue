    ...
    <footer class="bg-white">
        <div class="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
            <div
                ...
                <strong class="text-xl text-white sm:text-xl"> Start shopping! </strong>
                ...
                <NuxtLink :to="'/store'"
                    ...
                    <span class="text-sm font-medium">Let's go</span>
            ...
            <div class="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
                <div v-for="item in block" :key="item.id" class="text-center sm:text-left">
                    <p class="text-lg font-medium text-gray-900">{{ item.title }}</p>

                    <ul class="mt-8 space-y-4 text-sm">
                        <li v-for="(i, index ) in item.links" :key="index">
                            <a v-if="i !== 'Live chat'" class="text-gray-700 transition hover:text-gray-700/75"> {{ i }}
                            </a>

                            <a v-else :class="item.style" href="#">
                                <span class="text-gray-700 transition group-hover:text-gray-700/75"> {{ i }} </span>
            ...
            <div class="mt-16">
                <ul class="flex justify-center gap-6 sm:justify-end">
                    <li v-for="item in cosial_name" :key="item.id">
                        <a href="#" rel="noreferrer" target="_blank"
                            class="text-teal-700 transition hover:text-teal-700/75">
                            <span class="sr-only">{{ item.title }}</span>
                            <svg :key="item.id" class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24"
                                aria-hidden="true">
                ...
                <div class="mt-16 sm:flex sm:items-center sm:justify-between">
                    <NuxtLink :to="'/'" class="flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24">
                            <title>sofa-single-outline</title>
                            <path
...
const block = [
    ...
    },
    {
        id: 2,
        title: 'Our Services',
        links: ['Web Development', 'Web Design', 'Marketing', 'Google Ad']
    },
    {
        id: 3,
        title: 'Resources',
        links: ['Online Guides', 'Conference Notes', 'Forum', 'Downloads', 'Upcoming Events']
    },
    {
        id: 4,
        title: 'Helpful Links',
        links: ['FAQs', 'Support', 'Live chat'],
        style: 'group flex justify-center gap-1.5 ltr:sm:justify-start',
        newblock: true
    }
]
